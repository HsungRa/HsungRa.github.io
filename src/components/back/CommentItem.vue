<template>
  ...
  <div v-if="showReplies && comment.replies.length" class="replies">
    <CommentItem v-for="reply in comment.replies" :key="reply.id" :comment="reply" @reply="replyToComment" />
  </div>
</template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const loadReplies = (param) => {
      ...
      loadCommentDiscuss(commentNo).then(res => {
        ...
        for (let comment of comments) {
          ...
          if (comment.comments) {
            item['replies'] = loadReplies(comment)
          }
          result.push(item)
        }
      })
    ...
    // watch(() => props.comment, (ncomment) => {
    //   props.comment['replies'] = loadReplies(ncomment)
    // })
